...
impl UdpConn {
    ...
    /// TODO(matheus23) make private again
...
impl AsyncUdpSocket for UdpConn {
    ...
    fn try_send(&self, transmit: &Transmit<'_>) -> io::Result<()> {
        tracing::trace!(dst = %transmit.destination, ecn = ?transmit.ecn, len = transmit.contents.len(), segment_size = ?transmit.segment_size, src_ip = ?transmit.src_ip, "UDP send");
